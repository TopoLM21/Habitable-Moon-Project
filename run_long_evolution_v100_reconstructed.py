#!/usr/bin/env python3
"""Reconstructed v0.10 checkpoint/resume runner.

Built from the surviving v0.9.7 source plus preserved v0.9.8-v0.10 reports.
...
from tectonics.lithosphere import CrustType,advance_lithosphere,boundary_records_for_state,initialize_lithosphere,continental_material_fields
...
def parse_args():
    p=argparse.ArgumentParser(description='reconstructed v0.10 conservative long integration')
    p.add_argument('--config',default='configs/canonical_moon.yaml')
    p.add_argument('--output',default='outputs_v100_reconstructed')
...
def build_checkpoint(state,cycle,thermal,topo,system,baseline,manager,initial_cont_frac,initial_cont_vol,topo_rows,lithosphere_rows,relief_rows,cycle_rows,thermal_rows,late_rows,events,mantle_flow,transport_state):
    return RunCheckpoint(
        state,cycle,thermal,topo,system,baseline,manager,initial_cont_frac,initial_cont_vol,
        topo_rows,lithosphere_rows,relief_rows,cycle_rows,thermal_rows,events,late_rows,
        mantle_flow,transport_state
    )
...
def main():
    ...
    dyn0=dc(DynamicsParameters,cfg['plate_dynamics']);topop=dc(TopographyParameters,cfg['topography']);topp=dc(PlateTopologyParameters,cfg['plate_topology']);cc0=dc(ContinentalCycleParameters,cfg['continental_cycle']);thp=dc(ThermalParameters,cfg['thermal']);latep=dc(LateTectonicsParameters,cfg.get('late_tectonics',{}));mantlep=dc(MantleFlowParameters,cfg.get('mantle_flow',{}));transportp=dc(SubgridTransportParameters,cfg.get('subgrid_transport',{}))
    dt=float(a.dt if a.dt is not None else evo['time_step_myr']);out=Path(a.output);frames=out/'frames';plate_frames=out/'plate_frames';continental_frames=out/'continental_frames';checkpoints=out/'checkpoints';frames.mkdir(parents=True,exist_ok=True);plate_frames.mkdir(parents=True,exist_ok=True);continental_frames.mkdir(parents=True,exist_ok=True);checkpoints.mkdir(parents=True,exist_ok=True)
    ...
    if a.resume:
        ...
        if mantle_flow is None or transport_state is None:
            raise ValueError('v0.10 reconstructed resume requires a v0.10 reconstructed checkpoint; start a fresh run when upgrading from v0.9.7')
    ...
    def save_animation_frames():
        ...
        print('Frames:',fp.resolve(),pp.resolve(),cpng.resolve())
