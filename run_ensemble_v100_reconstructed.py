#!/usr/bin/env python3
"""Small multi-seed ensemble driver for the reconstructed v0.10 model.

The preserved v0.10 diagnosis explicitly recommends an ensemble before any
...
def parse_args():
    ...
    p.add_argument('--output', default='ensemble_v100_reconstructed')
...
def main():
    ...
    for seed in seeds:
        ...
        while current < float(a.end_time)-1e-9:
            ...
            cmd=[sys.executable,'run_long_evolution_v100_reconstructed.py','--config',str(cfg_path),'--output',str(seed_dir/'run'),'--end-time',str(target),'--dt',str(a.dt),'--checkpoint',str(cp)]
    ...
    # Compact aggregate useful for deciding whether the v0.10 WATCH items are systematic.
