#!/usr/bin/env python3
"""Moon Tectonics v0.18 persistent 2.5-D subduction-memory checkpoint/resume runner.
...
def parse_args():
    p=argparse.ArgumentParser(description='v0.18 subduction memory + v0.17 ridge push')
...
def build_checkpoint(state,cycle,thermal,topo,system,baseline,manager,initial_cont_frac,initial_cont_vol,topo_rows,lithosphere_rows,relief_rows,cycle_rows,thermal_rows,late_rows,events,mantle_flow,transport_state,hydrosphere,hydrosphere_rows,subduction_memory,subduction_memory_rows):
    return RunCheckpoint(
        ...
        mantle_flow=mantle_flow,transport_state=transport_state,hydrosphere=hydrosphere,hydrosphere_rows=hydrosphere_rows,subduction_memory=subduction_memory,subduction_memory_rows=subduction_memory_rows,
    )
...
def main():
    ...
    dyn0=dc(DynamicsParameters,cfg['plate_dynamics']);topop=dc(TopographyParameters,cfg['topography']);topp=dc(PlateTopologyParameters,cfg['plate_topology']);cc0=dc(ContinentalCycleParameters,cfg['continental_cycle']);thp=dc(ThermalParameters,cfg['thermal']);latep=dc(LateTectonicsParameters,cfg.get('late_tectonics',{}));mantlep=dc(MantleFlowParameters,cfg.get('mantle_flow',{}));transportp=dc(SubgridTransportParameters,cfg.get('subgrid_transport',{}));hydrop=dc(HydrosphereParameters,cfg.get('hydrosphere',{}));subp=dc(SubductionMemoryParameters,cfg.get('subduction_memory',{}))
    ...
    cp=build_checkpoint(state,cycle,thermal,topo,system,baseline,manager,initial_cont_frac,initial_cont_vol,topo_rows,lithosphere_rows,relief_rows,cycle_rows,thermal_rows,late_rows,events,mantle_flow,transport_state,hydrosphere,hydrosphere_rows,subduction_memory,subduction_memory_rows)
    ...
    if a.finalize:
        ...
        with (out/'summary_v118.json').open('w',encoding='utf-8') as h:json.dump(summary,h,ensure_ascii=False,indent=2)
