#!/usr/bin/env python3
"""Moon Tectonics v0.20 collision-triggered slab breakoff runner.
...
def parse_args():
    p=argparse.ArgumentParser(description='v0.20 collision-triggered slab breakoff')
...
def build_checkpoint(state,cycle,thermal,topo,system,baseline,manager,initial_cont_frac,initial_cont_vol,topo_rows,lithosphere_rows,relief_rows,cycle_rows,thermal_rows,late_rows,events,mantle_flow,transport_state,hydrosphere,hydrosphere_rows,subduction_memory,subduction_memory_rows,rollback_rows,breakoff_rows):
    return RunCheckpoint(
        ...
        mantle_flow=mantle_flow,transport_state=transport_state,hydrosphere=hydrosphere,hydrosphere_rows=hydrosphere_rows,subduction_memory=subduction_memory,subduction_memory_rows=subduction_memory_rows,rollback_rows=rollback_rows,breakoff_rows=breakoff_rows,
    )
...
def main():
    ...
    if a.finalize:
        ...
        with (out/'summary_v120.json').open('w',encoding='utf-8') as h:json.dump(summary,h,ensure_ascii=False,indent=2)
