#!/usr/bin/env python3
"""Moon Tectonics v0.21 slab-geometry volcanic-arc runner.
...
def parse_args():
    p=argparse.ArgumentParser(description='v0.21 slab-geometry volcanic arcs')
    p.add_argument('--config',default='configs/canonical_moon.yaml')
    p.add_argument('--output',default='outputs_v120_slab_breakoff')
...
def build_checkpoint(state,cycle,thermal,topo,system,baseline,manager,initial_cont_frac,initial_cont_vol,topo_rows,lithosphere_rows,relief_rows,cycle_rows,thermal_rows,late_rows,events,mantle_flow,transport_state,hydrosphere,hydrosphere_rows,subduction_memory,subduction_memory_rows,rollback_rows,breakoff_rows,arc_rows):
    return RunCheckpoint(
        ...
        mantle_flow=mantle_flow,transport_state=transport_state,hydrosphere=hydrosphere,hydrosphere_rows=hydrosphere_rows,subduction_memory=subduction_memory,subduction_memory_rows=subduction_memory_rows,rollback_rows=rollback_rows,breakoff_rows=breakoff_rows,arc_rows=arc_rows,
    )
...
def main():
    ...
    for dti in step_sizes(float(state.time_myr),end,dt):
        ...
        topo,last_relief,target=advance_topography(proto.mesh,state,boundaries,topo,dti,radius,topop,arc_uplift_forcing=topo_arc_forcing)
    ...
    if a.finalize:
        ...
        target,_=equilibrium_elevation(proto.mesh,state,boundaries,topop,radius,final_arc_forcing)
        ...
        with (out/'summary_v121.json').open('w',encoding='utf-8') as h:json.dump(summary,h,ensure_ascii=False,indent=2)
