#!/usr/bin/env python3
"""Run v0.8: dynamic plate topology + tectonic relief + continental crust life cycle."""
...
def parse_args():
    p=argparse.ArgumentParser(description='v0.8 continental crust life cycle')
...
def main():
    a=parse_args();cfg=load_config(a.config);proto=build_prototype(cfg);lc=cfg['lithosphere'];tc=cfg['tides'];evo=cfg['continental_evolution']
    dynp=dc(DynamicsParameters,cfg['plate_dynamics']);topop=dc(TopographyParameters,cfg['topography']);topp=dc(PlateTopologyParameters,cfg['plate_topology']);ccp=dc(ContinentalCycleParameters,cfg['continental_cycle'])
    ...
    out=Path(a.output or cfg['output'].get('continental_directory','outputs_v08'));frames=out/'frames';frames.mkdir(parents=True,exist_ok=True)
    ...
    cycle=initialize_continental_cycle(proto.mesh)
    ...
    manager=PlateTopologyManager(topp);times=snapshot_times(duration,interval);topo_rows=[];relief_rows=[];cycle_rows=[];events=[];frame_paths=[];last_td=None
    initial_areas=proto.mesh.physical_cell_areas_km2(radius);initial_cont_frac=float(np.sum(initial_areas[state.crust_type==int(CrustType.CONTINENTAL)])/np.sum(initial_areas))
    initial_cont_vol=float(np.sum(initial_areas[state.crust_type==int(CrustType.CONTINENTAL)]*state.crust_thickness_km[state.crust_type==int(CrustType.CONTINENTAL)]))
    for fi,tgt in enumerate(times):
        ...
        boundaries=boundary_records_for_state(proto.mesh,state,system,radius,normal,inactive)
        fp=frames/f'frame_{fi:04d}_{tgt:08.3f}_Myr.png';save_topology_frame(proto.mesh,state,topo,system,boundaries,last_td,fp,int(cfg['output'].get('continental_dpi',120)));frame_paths.append(fp)
        contf=100*float(np.sum(initial_areas[state.crust_type==int(CrustType.CONTINENTAL)])/np.sum(initial_areas))
        print(f"t={state.time_myr:7.2f} Myr | plates={len(system.plates):2d} | continent={contf:5.2f}% | relief={np.min(topo.elevation_m):7.0f}..{np.max(topo.elevation_m):7.0f} m")
    ...
    if cycle_rows:
        save_continental_cycle_history(cycle_rows,out/'continental_cycle_history.png');save_continental_flux_history(cycle_rows,out/'continental_flux_history.png')
        with (out/'continental_cycle_history.csv').open('w',newline='',encoding='utf-8') as h:w=csv.DictWriter(h,fieldnames=list(cycle_rows[0].keys()));w.writeheader();w.writerows(cycle_rows)
    ...
    counts=np.bincount(state.cell_plate,minlength=len(system.plates));cont_mask=state.crust_type==int(CrustType.CONTINENTAL);final_cont_frac=float(np.sum(initial_areas[cont_mask])/np.sum(initial_areas));final_cont_vol=float(np.sum(initial_areas[cont_mask]*state.crust_thickness_km[cont_mask]))
    summary={'version':'0.8','model':'v0.7 dynamic topology + tectonic relief + continental crust life cycle','duration_myr':duration,'time_step_myr':dt,'frame_interval_myr':interval,'mesh_cells':proto.mesh.cell_count,'initial_plate_count':len(proto.plates.plates),'final_plate_count':len(system.plates),'topology_event_count':len(events),'split_count':sum(e['kind']=='split' for e in events),'merge_count':sum(e['kind']=='merge' for e in events),'absorbed_small_plate_count':sum(e['kind']=='absorb' for e in events),'initial_continental_area_fraction':initial_cont_frac,'final_continental_area_fraction':final_cont_frac,'initial_continental_volume_km3':initial_cont_vol,'final_continental_volume_km3':final_cont_vol,'cumulative_juvenile_continental_area_created_km2':cycle.cumulative_generated_area_km2,'cumulative_continental_area_recycled_km2':cycle.cumulative_recycled_area_km2,'cumulative_continental_volume_generated_km3':cycle.cumulative_generated_volume_km3,'cumulative_continental_volume_recycled_km3':cycle.cumulative_recycled_volume_km3,'max_felsic_potential_final':float(np.max(cycle.felsic_potential)),'min_elevation_m':float(np.min(topo.elevation_m)),'max_elevation_m':float(np.max(topo.elevation_m)),'events':events,'notes':['Ocean-ocean subduction builds a persistent felsic island-arc potential before juvenile continental crust can appear.','Ocean-continent subduction thickens the overriding continental arc instead of directly creating large new land areas.','Damaged convergent continental margins can be tectonically eroded and recycled if they become too thin.','Over-thickened continental crust delaminates lower-crustal volume without automatically destroying the surface continent.','Felsic maturation is semi-Lagrangian advected with plate material between topology events.','All continental-cycle rates are effective prototype parameters, not a full melting/petrology solver.']}
    with (out/'summary_v08.json').open('w',encoding='utf-8') as h:json.dump(summary,h,ensure_ascii=False,indent=2)
