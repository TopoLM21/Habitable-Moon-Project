#!/usr/bin/env python3
"""Multi-seed ensemble driver for Moon Tectonics v0.11 material-layer model.
...
def parse_args():
    ...
    p.add_argument('--output', default='ensemble_v110_material')
...
def main():
    ...
    for seed in seeds:
        ...
        while current < float(a.end_time)-1e-9:
            ...
            cmd=[sys.executable,'run_long_evolution_v110.py','--config',str(cfg_path),'--output',str(seed_dir/'run'),'--end-time',str(target),'--dt',str(a.dt),'--checkpoint',str(cp)]
