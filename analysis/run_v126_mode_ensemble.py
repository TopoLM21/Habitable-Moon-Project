#!/usr/bin/env python3
"""Run the v0.26 four-mode plume-rifting validation ensemble locally."""
...
MODES = {
    ...
    "weakening_only": (True, True, False),
    "forcing_only": (True, False, True),
    "combined": (True, True, True),
}


def _prepare_config(seed: str, mode: str) -> Path:
    source = SEED_CONFIGS[seed]
    config = yaml.safe_load(source.read_text(encoding="utf-8"))
    plume_enabled, weakening_enabled, rifting_enabled = MODES[mode]
    config = deepcopy(config)
    config["mantle_plumes"]["enabled"] = plume_enabled
    config["mantle_plumes"]["lithosphere_weakening_enabled"] = weakening_enabled
    config["plume_rifting"]["enabled"] = rifting_enabled
    generated = RESULTS / "v126_mode_configs"
...
def _output_dir(seed: str, mode: str, end_time: float) -> Path:
    time_label = f"{end_time:g}".replace(".", "p")
    return RESULTS / f"v126_{mode}_sub3_{time_label}_seed_{seed}"
...
def _run_one(seed: str, mode: str, end_time: float, dt: float, force: bool) -> str:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"seed={seed} mode={mode} failed with exit code {completed.returncode}; "
...
def main() -> None:
    ...
    jobs = max(1, min(int(args.jobs), len(args.seeds) * len(args.modes)))
