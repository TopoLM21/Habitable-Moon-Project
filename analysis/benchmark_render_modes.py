...
def main():
    ...
    parser.add_argument("--modes", type=int, nargs="+", choices=(1, 2, 4), default=[1, 2, 4])
    ...
    with performance_core_benchmark(args.performance_cores) as affinity:
        ...
        for repeat in range(args.repeat):
            modes = [(workers, cells) for workers in args.modes
                     for cells in ([False, True] if args.compare_cell_kernels else [False])]
            if repeat % 2:
                modes.reverse()
            for workers, cells in modes:
                suffix = f"_cells{int(cells)}" if args.compare_cell_kernels else ""
                case = output / f"render{workers}_{repeat + 1}{suffix}"
                case.mkdir()
                ...
                print(f"START render{workers} cells={cells} repeat {repeat + 1}", flush=True)
                began = perf_counter()
                ...
                row = {"render_workers": workers, "cell_kernels": cells, "repeat": repeat + 1, "wall_seconds": wall,
