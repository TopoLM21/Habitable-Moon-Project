#!/usr/bin/env python3
"""Run paired multi-seed and subdivision-4 v0.29 validation integrations."""
...
def _prepare_config(seed: str, mode: str, subdivision: int) -> Path:
    ...
    config["hotspot_tracks"] = {
        "enabled": mode == "full",
        "head_tail_separation_enabled": mode == "full",
        "magmatic_thermal_weakening_enabled": mode == "full",
        "couple_thermal_to_lithosphere": mode == "full",
        "dike_localization_enabled": mode == "full",
        "underplate_evolution_enabled": mode == "full",
    }
    generated = RESULTS / "v129_validation_configs"
...
def _run(seed: str, mode: str, subdivision: int, end_time: float,
    ...
    summary = output / "summary_v129.json"
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "run_long_evolution_v129.py"),
            "--config", str(config),
