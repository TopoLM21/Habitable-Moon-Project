#!/usr/bin/env python3
"""Moon Tectonics v0.16 crust / mantle-lithosphere split checkpoint/resume runner.
...
def parse_args():
    p=argparse.ArgumentParser(description='v0.16 crust / mantle-lithosphere split + v0.15 material-aware topography')
    p.add_argument('--config',default='configs/canonical_moon.yaml')
    p.add_argument('--output',default='outputs_v116_material_topography')
...
def main():
    ...
    if a.finalize:
        ...
        save_lithosphere_split_maps(proto.mesh,state,out,int(cfg['output'].get('dpi',180)));save_lithosphere_split_history(lithosphere_rows,out/'lithosphere_split_history.png')
        ...
        with (out/'summary_v116.json').open('w',encoding='utf-8') as h:json.dump(summary,h,ensure_ascii=False,indent=2)
